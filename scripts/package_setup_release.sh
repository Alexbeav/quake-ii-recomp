...
#
# Writes: dist/quake-ii-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix quake-ii-recomp \
  --exe-name Quake_II_Recompiled \
  --display-name "Quake II Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Quake II disc" \
